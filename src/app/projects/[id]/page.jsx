...
//     // Find the single project that matches the ID from the URL
//     const project = projects.find(p => p.id === parseInt(props.params.id));

//     // If no project is found, you can render a "not found" message or a redirect
//     if (!project) {
//         return (
        ...
        <div className="w-full font-sans flex justify-center text-gray-800">
            <div className="w-8/12 max-md:10/12 max-md:w-full px-6">
                <div className="w-full" key={project.id}>
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-beige-500 my-4">Project Gallery</h3>
                        <div className="grid grid-cols-2 max-md:grid-cols-1 gap-4 pb-4">
                            {project.photos.map((photo, i) => (
                                <div 
            ...
            {isOpen && (
                ...
                >
                    <button 
                        onClick={closeModal} 
                        className="font-bold rounded-full w-9 h-9 z-40 flex justify-center items-center bg-white text-black opacity-80 hover:opacity-100 cursor-pointer absolute top-4 right-4 text-xl"
                    >
                        X
                    </button>
                    ...
                    <button 
                        ...
                        className="z-20 absolute left-2 md:left-4 p-2 md:p-3 bg-white text-black rounded-full opacity-70 hover:opacity-100 transition-opacity duration-300"
                    >
                    ...
                    <button 
                        ...
                        className="z-20 absolute right-2 md:right-4 p-2 md:p-3 bg-white text-black rounded-full opacity-70 hover:opacity-100 transition-opacity duration-300"
                    >
                        <FaChevronRight className="text-xl md:text-2xl" />
                    </button>
                    
                    <div 
                        className="relative h-full w-fit flex items-center justify-center p-4"
